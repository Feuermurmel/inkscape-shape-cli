...
from . import util
from .inkscapeextensions import inkex, simpletransform, cubicsuperpath, \
    cspsubdiv
...
class DXFExportEffect(inkex.Effect):
    ...
    def __init__(self, layers, flatness):
    ...
    def _check_document_units(self):
        height_attr = self.document.getroot().get('height')

        help_url = 'https://github.com/Feuermurmel/openscad-template/wiki/Absolute-Measurements'

        if height_attr is None:
            raise util.UserError(
                f'SVG document has no height attribute. See {help_url}')

        _, height_unit = self._parse_measure(height_attr)

        if height_unit is None or height_unit == 'px':
            raise util.UserError(
                f'Height of SVG document is not an absolute measure. See {help_url}')

        if self.document.getroot().get('viewBox') is None:
            raise util.UserError(
                f'SVG document has no viewBox attribute. See {help_url}')

    def effect(self):
        self._check_document_units()
        ...
        layers_by_inkscape_name = {i.inkscape_name: i for i in self._layers}
    ...
    def write_dxf(self, out_path):
        with open(out_path, 'w', encoding='utf-8') as file:
            ...
            file.write(pkgutil.get_data(__name__, 'dxf_header.txt').decode())
            ...
            file.write(pkgutil.get_data(__name__, 'dxf_footer.txt').decode())
    
    def write_asy(self, file):
        def write_line(format, *args):
            print(format.format(*args) + ';', file=file)
    ...
    def _parse_measure(cls, string):
        ...
        unit_match = re.search('(%s)$' % '|'.join(list(cls._unit_factors.keys())), string)    
